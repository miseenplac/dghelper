...
18. **`renderCalibrationStatus()` calls from `runCalibrationForMetric` must be in the `finally` block, AFTER `_calActive = false`** — never inline in the `try` body. `renderCalibrationStatus` early-returns when `_calActive === true` (Phase 1 clobber guard, prevents the 1 s setInterval from overwriting countdown text mid-capture), so any inline call during the try body is a no-op. Symptom of an inline call: countdown text "Hover TOP-LEFT — 1s" sticks in the DOM for up to 1 s after the calibration exits (success or error), until the 1 s setInterval at the bottom of `index.js` fires the next render cycle. Most visible on error paths (mouse off RS window, region too small) where the user otherwise sees the plugin appear frozen on "1s". See commit `c9b99f4` for the fix and the comment block at [src/index.js runCalibrationForMetric finally](src/index.js).
